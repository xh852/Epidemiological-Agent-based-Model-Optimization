...
def distribute_random_vaccine(agent_list, int vaccine_availability_day, int daily_vaccine_distribution_count,float vaccine_efficacy=0.95, int current_day=0):
    """
...
def infect(agent_list, float infection_distance, float infection_probability, float infection_probability_increase = 0.4):
    ...
    cdef np.ndarray infected_locations = np.array([list(agent.location) for agent in agent_list if agent.status == "I"])
    cdef np.ndarray susceptible_agents = np.array([[agent.location[0],agent.location[1], agent] for agent in agent_list if agent.status == "S"])
    if len(infected_locations) == 0 or len(susceptible_agents) == 0:
        #Only proceed if infected AND susceptible agents exist
        return
    cdef np.ndarray susceptible_locations = susceptible_agents[:, :2].astype("float64")
    cdef np.ndarray susceptible_info = susceptible_agents[:, 2:]

    # Get distance between every infected and susceptible pair
    cdef np.ndarray distances = np.sqrt(np.sum((np.array(infected_locations)[:, np.newaxis, :] - np.array(susceptible_locations)[np.newaxis, :, :])**2, axis=-1))

    # Find indices of distances below the threshold and update susceptible agents
    cdef np.ndarray infectable_agents = np.argwhere(distances < infection_distance)
